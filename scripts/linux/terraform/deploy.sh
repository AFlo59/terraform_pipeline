#!/bin/bash
# =============================================================================
# Deploy Script - Déploiement par environnement
# =============================================================================
...
if [[ -z "$ENV" ]] || [[ ! "$ENV" =~ ^(dev|rec|prod)$ ]]; then
    ...
    echo ""
    echo "Environnements:"
    echo "  dev   - Développement"
    echo "  rec   - Recette (staging)"
    echo "  prod  - Production"
    echo ""
    echo "Actions:"
    echo "  plan    - Prévisualiser les changements (défaut)"
...
# Vérifications
if [ ! -f "$ENV_FILE" ]; then
    echo -e "${RED}[ERROR]${NC} Fichier d'environnement non trouvé: $ENV_FILE"
    exit 1
fi
...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║           Terraform Deployment - NYC Taxi Pipeline               ║"
echo "╚══════════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
if [[ "$ACTION" == "destroy" && "$ENV" == "prod" ]]; then
    echo -e "${RED}⚠️  ATTENTION: Vous êtes sur le point de DÉTRUIRE l'environnement PRODUCTION !${NC}"
    read -p "Tapez 'DESTROY PROD' pour confirmer: " CONFIRM
    if [[ "$CONFIRM" != "DESTROY PROD" ]]; then
        echo -e "${YELLOW}[CANCELLED]${NC} Opération annulée"
        exit 0
    fi
fi
...
if [ -f "$SECRETS_FILE" ]; then
    ...
    echo -e "${YELLOW}[WARNING]${NC} Fichier secrets.tfvars non trouvé"
    echo "Créez-le avec: cp environments/secrets.tfvars.example environments/secrets.tfvars"
fi
